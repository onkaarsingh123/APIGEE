//This functions checks if a resource is present in the Profile Object.
function getEmr(profileObject) {
    var len = profileObject.length;
    var emr;
    
    for (i = 0; i < len; i++) {
        if (profileObject[i].resource_name == primaryResource ) {
            emr = profileObject[i].server_name;
            return emr
        }
    }
    for (i = 0; i < len; i++) {
        if (profileObject[i].resource_name == "default") {
            emr = profileObject[i].server_name;
            return emr
        }
    }
...
if (validScopeFound) {
	...
	var emr;
    try{
    var profileString = context.getVariable('profileObject');
    var profileObject = JSON.parse(profileString);
	emr = getEmr(profileObject);
    }
    catch(Error ){
        print("JSON Object parse Error!");
    }

    print("Emr Hit....  " + emr);
    if(emr === "openepic")
		context.setVariable("connector_basepath", "openepic");
	else if(emr === "cerner")
		context.setVariable("connector_basepath", "cerner");
	else if(emr === "openemr")
		context.setVariable("connector_basepath", "openemr/api");	
	else 
		context.setVariable("connector_basepath", "hapi-dstu2");	
